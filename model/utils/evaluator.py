...
def cuda_partAvg_dist(x, y, part_dim):
    x = torch.from_numpy(x).cuda()
    y = torch.from_numpy(y).cuda()
    feature_dim = x.size(1)
    n_x = x.size(0)
    n_y = y.size(0)
    part_num = int(feature_dim / part_dim)
    dist = torch.zeros(n_x, n_y).cuda()
    for i in range(part_num):
        _x = x[:, i*part_dim:(i+1)*part_dim]
        _y = y[:, i*part_dim:(i+1)*part_dim]
        ...
        dist = dist + _dist
    return dist / part_num

def evaluation(data, dataset, part_dim=False):
            ...
            for (v1, probe_view) in enumerate(view_list):
                for (v2, gallery_view) in enumerate(view_list):
                    ...
                    if part_dim == False:
                        ...
                        dist = cuda_partAvg_dist(probe_x, gallery_x, part_dim)

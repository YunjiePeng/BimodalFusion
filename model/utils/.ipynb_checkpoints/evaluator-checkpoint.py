...
def cuda_dist(x, y):
    ...
    n_x = x.size(0)
    n_y = y.size(0)
    part_num = int(feature_dim / 256)
    
    dist = torch.zeros(n_x, n_y).cuda()
    for i in range(part_num - 1):
        _x = x[:, i*256:(i+1)*256]
        _y = y[:, i*256:(i+1)*256]
        _dist = torch.sum(_x ** 2, 1).unsqueeze(1) + torch.sum(_y ** 2, 1).unsqueeze(1).transpose(0, 1) - 2 * torch.matmul(_x, _y.transpose(0, 1))
        _dist = torch.sqrt(F.relu(_dist))
        dist = dist + _dist
    sil_dist = dist / (part_num - 1)
    
    _x = x[:, -256:]
    _y = y[:, -256:]
    ske_dist = torch.sum(_x ** 2, 1).unsqueeze(1) + torch.sum(_y ** 2, 1).unsqueeze(1).transpose(0, 1) - 2 * torch.matmul(_x, _y.transpose(0, 1))
    
#     print("sil_dist:", torch.mean(sil_dist)) # 2.9~3.0
#     print("ske_dist:", torch.mean(ske_dist)) # 0.3~0.4
    
                                                                                                                                                
    ske_weight = 0.8
    fusion_dist = sil_dist + ske_weight * ske_dist
    return fusion_dist

...
class MSGG_1Layer(nn.Module):
    ...
    def forward(self, x, batch_frame=None):
        ...
        if self.edge_importance_weighting:
            for gcn_lowSemantic, importance_lowSemantic in zip(self.st_gcn_networks_lowSemantic, self.edge_importance_lowSemantic):
                x, _ = gcn_lowSemantic(x, self.A_lowSemantic * importance_lowSemantic)
        else:
            for gcn_lowSemantic in self.st_gcn_networks_lowSemantic:
        ...
        x_fc = self.fc(x.view(N, -1))
        bn_x_fc = self.bn_neck(x_fc)
        x_cls_score = self.encoder_cls(bn_x_fc)

        x_fc = x_fc.unsqueeze(1).contiguous()
        return x, x_fc, x_cls_score

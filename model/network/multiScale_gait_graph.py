...
class MSGG(nn.Module):
    ...
    def __init__(self,
        ...
        if edge_importance_weighting:
            ...
            self.edge_importance_highSemantic = [1] * len(self.st_gcn_networks_highSemantic)

        self.fc = nn.Linear(out_channels, out_channels)
        self.bn_neck = nn.BatchNorm1d(out_channels)
        self.encoder_cls = nn.Linear(out_channels, num_id, bias=False)

        # #---For Ablation Study: Joints+Limbs+Bodyparts (Separate)---
        # self.fc = nn.Linear(3*out_channels, 3*out_channels)
        # self.bn_neck = nn.BatchNorm1d(3*out_channels)
        # self.encoder_cls = nn.Linear(3*out_channels, num_id, bias=False)
    ...
    def forward(self, x, batch_frame=None):
        ...
        if self.edge_importance_weighting:
            for gcn_lowSemantic, importance_lowSemantic, gcn_mediumSemantic, importance_mediumSemantic, gcn_highSemantic, importance_highSemantic in zip(self.st_gcn_networks_lowSemantic, self.edge_importance_lowSemantic, self.st_gcn_networks_mediumSemantic, self.edge_importance_mediumSemantic, self.st_gcn_networks_highSemantic, self.edge_importance_highSemantic):
                ...
                #---Cross-scale Message Passing, Turn off for Ablation Study: Joints+Limbs+Bodyparts (Separate)---
                x_sp = self.semantic_pooling(x)
                y = torch.add(y, x_sp)
                y_sp = self.semantic_pooling(y)
                z = torch.add(z, y_sp)
        else:
            for gcn_lowSemantic, gcn_mediumSemantic, gcn_highSemantic in zip(self.st_gcn_networks_lowSemantic, self.st_gcn_networks_mediumSemantic, self.st_gcn_networks_highSemantic):
                x, _ = gcn_lowSemantic(x, self.A_lowSemantic)
                y, _ = gcn_mediumSemantic(y, self.A_mediumSemantic)
                z, _ = gcn_highSemantic(z, self.A_highSemantic)
        ...
        N, C, T, V = y_sp.size()
        y_sp = y_sp.permute(0, 2, 3, 1).contiguous()
        y_sp = y_sp.view(N, T*V, C)
        ...
        return x_sp, y_sp, z, z_fc, z_cls_score
        ...
        # concat_feature = torch.cat([x.view(N, -1), y.view(N, -1), z.view(N, -1)], -1)
        # concat_feature_fc = self.fc(concat_feature)
        # concat_feature_fc_bn = self.bn_neck(concat_feature_fc)
        # concat_feature_cls_score = self.encoder_cls(concat_feature_fc_bn)
        # concat_feature_fc = concat_feature_cls_score.unsqueeze(1).contiguous()
        # return x, y, z, concat_feature_fc, concat_feature_cls_score

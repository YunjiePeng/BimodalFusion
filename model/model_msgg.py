...
class Model_MSGG:
    def __init__(self,
        ...
        self.encoder = MSGG(**model_cfg).float()
        ...
        self.triplet_loss_z = TripletLoss(self.P * self.M, self.hard_or_full_trip, self.margin).float()

        self.cross_entropy_loss_z = nn.CrossEntropyLoss().float()
        ...
        self.triplet_loss_z = DistributedLossWrapper(self.triplet_loss_z)
        self.triplet_loss_z.cuda(self.device)

        self.cross_entropy_loss_z = DistributedLossWrapper(self.cross_entropy_loss_z)
        self.cross_entropy_loss_z.cuda(self.device)
        ...
        self.hard_loss_metric_z = []
        self.full_loss_metric_z = []
        self.full_loss_num_z = []
        self.dist_list_z = []
        self.mean_dist_z = 0.01

        self.cross_entropy_loss_metric_z = []
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            for i in range(len(seq)):
                ...
                # logging.info("seq[i][0][0]:{}".format(seq[i][0][0]))
            if batch_frame is not None:
                batch_frame = self.np2var(batch_frame).int()

            feature_x, feature_y, feature_z, feature_z_fc, z_cls_score = self.encoder(*seq, batch_frame)
            ...
            triplet_feature_y = feature_y.permute(1, 0, 2).contiguous()
            ...
            triplet_feature_z = feature_z_fc.permute(1, 0, 2).contiguous()
            triplet_label_z = target_label.unsqueeze(0).repeat(triplet_feature_z.size(0), 1)
            (full_loss_metric_z, hard_loss_metric_z, mean_dist_z, full_loss_num_z
             ) = self.triplet_loss_z(triplet_feature_z, triplet_label_z, dim=1)

            cross_entropy_loss_metric = self.cross_entropy_loss_z(z_cls_score, target_label, dim=0)
            ...
            self.hard_loss_metric_z.append(hard_loss_metric_z.mean().data.cpu().numpy())
            self.full_loss_metric_z.append(full_loss_metric_z.mean().data.cpu().numpy())
            self.full_loss_num_z.append(full_loss_num_z.mean().data.cpu().numpy())
            self.dist_list_z.append(mean_dist_z.mean().data.cpu().numpy())

            self.cross_entropy_loss_metric_z.append(cross_entropy_loss_metric.mean().data.cpu().numpy())
            ...
            if self.restore_iter % 100 == 0:
                ...
                log_str = log_str + ', Layer1---hard_loss_metric={0:.8f}'.format(np.mean(self.hard_loss_metric_x))
                ...
                log_str = log_str + ', Layer2---hard_loss_metric={0:.8f}'.format(np.mean(self.hard_loss_metric_y))
                ...
                log_str = log_str + ', mean_dist={0:.8f}\n'.format(self.mean_dist_y)

                log_str = log_str + 'iter {}:'.format(self.restore_iter)
                log_str = log_str + ', Layer3---hard_loss_metric={0:.8f}'.format(np.mean(self.hard_loss_metric_z))
                log_str = log_str + ', full_loss_metric={0:.8f}'.format(np.mean(self.full_loss_metric_z))
                log_str = log_str + ', full_loss_num={0:.8f}'.format(np.mean(self.full_loss_num_z))
                self.mean_dist_z = np.mean(self.dist_list_z)
                log_str = log_str + ', mean_dist={0:.8f}'.format(self.mean_dist_z)
                log_str = log_str + ', softmax_loss={0:.8f}'.format(np.mean(self.cross_entropy_loss_metric_z))
    ...
    def transform(self, flag, batch_size=1):
        ...
        for i, x in enumerate(data_loader):
            ...
            _, _, _, feature, cls_score = self.encoder(*seq, batch_frame)

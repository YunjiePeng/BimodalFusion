...
class Model_MSGG1Layer:
    def __init__(self,
        ...
        self.encoder = MSGG_1Layer(**model_cfg).float()
        ...
        self.cross_entropy_loss_x = nn.CrossEntropyLoss().float()
        ...
        if state == 'train':
            ...
            self.cross_entropy_loss_x = DistributedLossWrapper(self.cross_entropy_loss_x)
            self.cross_entropy_loss_x.cuda(self.device)

        elif state == 'test':
            self.encoder.cuda()
            self.triplet_loss_x.cuda()
            self.cross_entropy_loss_x.cuda()
        ...
        self.cross_entropy_loss_metric_x = []
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            feature_x, feature_x_fc, x_cls_score = self.encoder(*seq, batch_frame)

            
            target_label = [train_label_set.index(l) for l in label]
            target_label = self.np2var(np.array(target_label)).long()

            triplet_feature_x = feature_x_fc.permute(1, 0, 2).contiguous()
            ...
            cross_entropy_loss_metric = self.cross_entropy_loss_x(x_cls_score, target_label, dim=0)
            ...
            if self.hard_or_full_trip == 'hard':
                ...
                loss = hard_loss_metric_x.mean() + cross_entropy_loss_metric.mean()
                ...
                loss = full_loss_metric_x.mean() + cross_entropy_loss_metric.mean()
            ...
            self.cross_entropy_loss_metric_x.append(cross_entropy_loss_metric.mean().data.cpu().numpy())
            ...
            if self.restore_iter % 100 == 0:
                ...
                log_str = log_str + ', mean_dist={0:.8f}'.format(self.mean_dist_x)
                log_str = log_str + ', softmax_loss={0:.8f}'.format(np.mean(self.cross_entropy_loss_metric_x))
    ...
    def transform(self, flag, batch_size=1):
        ...
        for i, x in enumerate(data_loader):
            ...
            if batch_frame is not None:
                batch_frame = self.np2var(batch_frame).int()
            _, feature, cls_score = self.encoder(*seq, batch_frame)

...
m = initialization(conf, model=opt.model, state='test', test=opt.cache, local_rank=opt.local_rank)[0]

if opt.ckp_prefix is None:
    logging.info('Loading the model of iteration %d...' % opt.iter)
    m.load(opt.iter)
else:
...
logging.info('Transformation complete. Cost:{}'.format(datetime.now() - time))
time = datetime.now()
logging.info('Evaluating...')
if opt.local_rank == 0:
    acc = evaluation(test, dataset, part_dim)
    logging.info('acc.shape:{}'.format(acc.shape))
    logging.info('Evaluation complete. Cost:{}'.format(datetime.now() - time))
